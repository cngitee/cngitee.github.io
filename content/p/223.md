+++
title = 'VB6校验文件防破解'
date = 2022-11-30T11:36:59+08:00
draft = false
tags = 'VB'
categories = 'VC#源码'
+++
...
 Dim str As String
str = IIf(Len(App.Path) > 3, App.Path & "\" & App.EXEName & ".exe", App.Path & App.EXEName & ".exe")
Print "MD5:" & HashFile(str) '获取exe文件的MD5校验码
Print "大小:" & FileLen(str) & " 字节"  '取当前体积大小
End Sub
```
### 2、新建一个MD5.bas模块
``` vb.net {linenos=inline}
